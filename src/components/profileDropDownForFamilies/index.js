...
const ProfileDropDownForFamilies = ({ country, dataArray, maintitle, label = "", placeholder, onSelect, selected = "" }) => {
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: country ? heightPixel(250) : heightPixel(350) }} nestedScrollEnabled={true} >
                            {
                                dataArray.map((item, index) => {
                                    return (
                                        <View key={index} style={{ paddingVertical: heightPixel(2), flexDirection: "row"}}>
                                            {item?.icon}
...
export default ProfileDropDownForFamilies;
